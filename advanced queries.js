...
async function runQueries() {
  ...
  try {
    ...
    // 🔍 6️⃣ Find books with price between $10 and $15
    const midRangeBooks = await books
      .find({ price: { $gte: 10, $lte: 15 } })
      .project({ title: 1, author: 1, price: 1, _id: 0 })
      .toArray();
    console.log('\nBooks priced between $10 and $15:', midRangeBooks);

    // 🔍 7️⃣ Use regex to find titles containing the word “The”
    const titleRegex = await books
      .find({ title: { $regex: /The/i } })
      .project({ title: 1, author: 1, _id: 0 })
      .toArray();
    console.log('\nBooks with "The" in the title:', titleRegex);

    // 🔍 8️⃣ Find out-of-stock books (for inventory check)
    const outOfStock = await books.find({ in_stock: false }).toArray();
    console.log('\nBooks currently out of stock:', outOfStock);

    // 🔍 9️⃣ Find top 3 most expensive books
    const top3Expensive = await books.find().sort({ price: -1 }).limit(3).toArray();
    console.log('\nTop 3 most expensive books:', top3Expensive);

    // 🔍 🔟 Count number of books per genre (simple query version)
    const genreCounts = await books.aggregate([
      { $group: { _id: '$genre', totalBooks: { $sum: 1 } } },
      { $sort: { totalBooks: -1 } }
    ]).toArray();
    console.log('\nTotal books by genre:', genreCounts);
    ...
    // 3️⃣ Use explain() to analyze performance
    const explainResult = await books.find({ title: '1984' }).explain('executionStats');
    console.log('\nExplain plan for title search:', explainResult.executionStats);
